    ...
    const fetchScholarship = async () => {
      try {
        ...
        if (data.success && data.data && data.data.length > 0) {
          ...
          setScholarship({
            id: sch.id,
            image: sch.image || '/scholarshipleft.png',
            title: sch.title || sch.name,
            description: sch.description || 'Learn more about this scholarship opportunity.',
          })
        }
      } catch (error) {
            ...
            {loading ? (
              ...
              <>
                ...
                <div className=" space-y-4 md:ml-4 w-full md:w-1/2">
                  <div>
                    <h3 className="text-xl md:text-2xl font-normal mt-4 text-foreground mb-2 text-[#393F50]">{scholarship.title}</h3>
                    <p className="text-muted-foreground line-clamp-3 text-sm md:text-base mt-4 text-[#393F50]">{scholarship.description}</p>
                  </div>
              ...
              <div className="flex items-center justify-center w-full py-12">
                <p className="text-muted-foreground">No scholarship highlights available at the moment.</p>
              </div>
            )}
          </div>
